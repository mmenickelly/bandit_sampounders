function [X_inc_array, nf_array, models] = strawman_sam_pounders_paper(Ffun, X_0, n, nf_max, g_tol, delta_0, m, Low, Upp, batch_size, expert_array, Prior, Options, Model)

%% THIS FUNCTION SHOULD NEVER BE USED. IT IS MEANT FOR A COMPARISON TO
%% A STRAWMAN METHOD FOR THE SAM POUNDERS PAPER. 
%% 
%% THIS IS JUST SAM POUNDERS BUT I'M RIPPING OUT EVERYTHING RELATED TO EXP4. 
...
if num_experts > 1
    error('This strawman method should only have an expert array of length 1.')
end
...
while nf < nf_max
    ...
    if any(isnan(combined_probs))
     ...
     % update the subset of models (and simultaneously compute reward - not
     % here)
     for j = to_update'
